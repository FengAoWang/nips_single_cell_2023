...
class CPAModule(BaseModuleClass):
    ...
    def mrrmse(self, tensors, inference_outputs, generative_outputs):
        x = tensors[CPA_REGISTRY_KEYS.X_KEY]
        px = generative_outputs['px']
        ...
        Y_true = x.cpu().detach().numpy().astype(np.float64)
        Y_pred = reconstruction.cpu().detach().numpy().astype(np.float64)
